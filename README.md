...
No ML model, no 1.2 GB download, no tokenizer dependency, no provider SDKs
...
|---|---|---|---|
| TypeScript (Node ≥ 18 / Bun) | [`ts/`](ts/README.md) | **zero** | `npm install leanctx` |
| Rust | [`rust/`](rust/README.md) | **zero** (`[dependencies]` empty) | `cargo add leanctx` |
| Go | [`go/`](go/README.md) | **zero** (stdlib only) | `go get github.com/itaides/leanctx/go` |
...
console.log(response.usage.leanctxTokensSaved);
```
...
Earlier versions of this repository contained a Python SDK built around
Microsoft's LLMLingua-2 model. This project replaces it with the
dependency-free design above; the Python implementation is available in the
