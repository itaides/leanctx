 ...
 *   - leanctxFetch custom-fetch middleware
 *   - wrap() duck-typed instance wrapper
 *
 * Asserts messages are compressed on the wire and usage.leanctx* telemetry is
 * attached on the response.
 */

import { afterAll, describe, expect, test } from "bun:test";
import { Anthropic } from "../src/anthropic.js";
import { leanctxFetch } from "../src/fetch.js";
import { OpenAI } from "../src/openai.js";
import type { LeanctxConfig } from "../src/types.js";
...
describe("minimal OpenAI client", () => {
    test("compresses request messages and attaches telemetry", async () => {
        ...
        expect(usage.leanctxMethod).toBe("extract");
        expect(usage.leanctxTokensSaved as number).toBeGreaterThan(0);
        expect(usage.leanctxRatio as number).toBeLessThan(1);
    });
...
describe("wrap() duck-typed wrapper", () => {
    ...
    test("intercepts chat.completions.create and messages.create", async () => {
        ...
        expect(r1.usage.leanctxMethod).toBe("extract");
        ...
        expect(r2.usage.leanctxMethod).toBe("extract");
    });
