...
type CompressorFactory = (config: LeanctxConfig) => Compressor;
...
export class Middleware {
    ...
    private buildRouter(config: LeanctxConfig): Router {
        ...
        for (const [ctypeStr, compressorName] of Object.entries(routing)) {
            if (!CONTENT_TYPE_VALUES.has(ctypeStr)) {
                console.warn(
                    `leanctx: unknown content type ${JSON.stringify(ctypeStr)} in routing config; ignored`,
                );
                continue;
            }
            ...
            if (factory === undefined) {
                console.warn(
                    `leanctx: compressor ${JSON.stringify(compressorName)} not available; ` +
...
function buildStrategies(config: LeanctxConfig): Strategy[] {
