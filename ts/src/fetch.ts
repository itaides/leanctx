/**
 * leanctxFetch — compression as a `fetch` middleware.
 *
 ...
 *
 *     const client = new OpenAI({ fetch: leanctxFetch({ mode: "on", ... }) });
 *
 ...
 * leanctxTokensSaved / leanctxRatio / leanctxMethod. Streaming responses are
...
export function leanctxFetch(
    ...
    const wrapped = async (
        ...
        annotated.leanctxTokensSaved = stats.inputTokens - stats.outputTokens;
        annotated.leanctxRatio = stats.ratio;
        annotated.leanctxMethod = stats.method;
