    ...
    pub fn new(config: Config) -> Self {
        ...
        for (ctype_str, compressor_name) in &config.routing {
            let Some(ctype) = ContentType::parse(ctype_str) else {
                eprintln!("leanctx: unknown content type {ctype_str:?} in routing config; ignored");
                continue;
            };
            let compressor: Box<dyn Compressor> = match compressor_name.as_str() {
                ...
                other => {
                    eprintln!(
                        "leanctx: compressor {other:?} not available; falling back to default (verbatim) for {ctype_str}"
                    );
                    continue;
                }
            };
